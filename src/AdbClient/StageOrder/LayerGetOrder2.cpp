...
void LayerGetOrder2::on_pushButton_clicked()
{
    //refresh();
    
    QTimer::singleShot(500,this,[=]{
        
        on_tbUser_cellPressed(0,0);
        
        QTimer::singleShot(500,this,[=]{
            on_tbOrder_cellPressed(0,0);
            emit ui->btnFinish->click();
        });
...
void LayerGetOrder2::uiWait(_LockStatus iType, QString sMsg)
{
...
#endif
        UI.showMsg("","回報完成\n\n"+m_sUnLockMsg,QStringList()<<"完成，並複製文字");

        UI.copyMsg(m_sUnLockMsg);

#if TEST_ACTION

        QTimer::singleShot(500,this,[=]{

            ui->pushButton->setText(QString::number(++iTesetCount));
    ...
    else if(unLockStatus!=_CHECK && unLockStatus!=_NO_CHECK  && unLockStatus!=_NOTE)
        UI.showMsg("",m_sUnLockMsg,"OK");

}

void LayerGetOrder2::timerEvent(QTimerEvent *)
{
